...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    if (empty($_POST['user_id_mod'])){
      ...
      && !empty($_POST['user_password_new3'])
        ...
        $nuser=$_SESSION['inst_firstname']." ".$_SESSION['inst_lastname'];
        ...
        $user_password_hash = password_hash($user_password, PASSWORD_DEFAULT);
          ...
          // write new user's data into database
                    $sql = "UPDATE instructores SET inst_password_hash='".$user_password_hash."' WHERE inst_id='".$user_id."'";
                    $query = mysqli_query($con,$sql);

                    // if user has been added successfully
                    if ($query) {
                        $messages[] = "La contraseña ha sido modificada con éxito.";
                          $nota="El instructor $nuser modificó su contraseña";                        
                        guardar_historial_instructores($user_id,1,$fecha,$nota);
                    } else {
